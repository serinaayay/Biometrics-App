...
export default function HomeScreen({ navigation }: Props) {
  ...
  // Always allow empty string or "N/A"
  ...
  const cleanedNum = text.replace(/[^0-9]/g, '');

  // If no digits left, let user clear it
  if (cleanedNum === '') {
    setSssNumber('');
  ...
  // Slice to 10 digits
  const sliced = cleanedNum.slice(0, 10);
  ...
  const saveUserToDatabase = async (completeUserData: any) => {
    try {
      ...
      console.log('Request body:', JSON.stringify(completeUserData));
                ...
                {jobChoice.map((option2, index) => (
                  <Pressable
                    ...
                    onPress={() => {
                      ...
                      // If "Others" is selected, set isOther to true
                      if (option2 === 'Others') {
                        setIsOther(true);
                      } else {
                        setIsOther(false);
